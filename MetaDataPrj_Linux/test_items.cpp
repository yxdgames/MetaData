...
void test_item_serial_bin(char *pFileName, char flag)
{
	if (flag)
	{
		...
		*g_test_items_derived1.mem_cls1.p_mem_long = 9284;
		g_test_items_derived1.mem_string.SetValue("abdlewov���Ĳ��ԡ���Ӣ���ַ���!��");
	...
	else
	{
		...
		*g_test_items_derived1.mem_cls1.p_mem_long = 923384;
		g_test_items_derived1.mem_string.SetValue("���Ĳ��ԡ���Ӣ���ַ���!��abdlewov~");
...
void test_item_metadata_other(void)
{
	...
	pStr = reinterpret_cast<SimpleString*>(
		reinterpret_cast<const CMetaDataClassType*>(TypeTraits<SimpleString>::GetMetaDataType())->CreateObject<SimpleString>(&PVector, "�ַ���ָ��\n")
	);
	if (pStr)
	{
		std::cout << pStr->GetValue();
