...
private:
	void *DoCreateObject(CFuncParamMDVector *pParamMDVector, ...) const;
#if defined(CO_OS_WIN)
	inline void *DoCreateObject(CFuncParamMDVector *pParamMDVector, va_list pParamList) const;
#elif defined(CO_OS_LINUX)
#ifdef CO_MACHINE_X64
	inline void *DoCreateObject(CFuncParamMDVector *pParamMDVector, uint64_t reg_params[], const int reg_param_num,
...
{
	void *pReturn;
	void *pObj(DoCreateObject(nullptr));
	if (pObj)
	{
		pReturn = this->AsType(pObj, TypeTraits<T>::GetMetaDataType());
		if (!pReturn)
