...
bool CMetaDataFunction::FuncParamsCheck(CFuncParamMDVector *pParamMDVector) const
{
	...
	{
		if (!m_pParamTable->at(index)->GetMDType()->Compare(pParamMDVector->at(index)->GetMDType())
									 ...
									 va_list pParamList, void *pReturn) const
{
	...
	if (param_count)
	{
		if (0 == param_addr) throw new ExceptionMetaData(D_E_ID_MD_META_DATA_OF_FUNC_CALL, "错误：参数表缺失！", true);
	...
	try
	{
		for (size_t index = 0; index < param_count; ++index)
		{
			if (pParamMDVector)
			{
				if (!pParamMDVector->at(index)->GetMDType()->Compare(m_pParamTable->at(index)->GetMDType())
									...
									uint8_t stack_params[], void *pReturn) const
{
	...
	if (param_count)
	{
		if (!reg_params) throw new ExceptionMetaData(D_E_ID_MD_META_DATA_OF_FUNC_CALL, "错误：参数表缺失！", true);
		pParamPtrBuffer = new void*[param_count];
		memset(pParamPtrBuffer, 0x00, sizeof(void*) * param_count);
	}
	try
		...
		if (bParamsOK)
		{
			...
			if (m_pReturnInfo)
			{
				...
				else
				{
					...
					{
						if (!stack_params) throw new ExceptionMetaData(D_E_ID_MD_META_DATA_OF_FUNC_CALL, "错误：参数表缺失！", true);
